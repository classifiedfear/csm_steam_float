...
class MatchingSkinGetter(metaclass=abc.ABCMeta):
    ...
    async def compare_all_data(self) -> AsyncIterable[dict] | None:
        ...
        parser_site_1_task = await parser_site_1.fill()
        if not parser_site_1_task:
            return

        parser_site_2_task = await parser_site_2.fill()
...
class CsmSteamMatchingDataGetter(MatchingSkinGetter):
    async def factory_method(self) -> Tuple[ParserInterface, ParserInterface]:
        ...
        return (
            parser_factory.create_parser('csm_tests'),
